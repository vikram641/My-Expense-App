...
import com.example.expense.feature.expense.add.AddExpenseFragment
...
//        val numeric = formats["numeric"]       // 04/2026
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    fun updateMonth(date: String?, action: Action, value: Long = 1): String {
        val formatter = DateTimeFormatter.ofPattern("yyyy-MM")
        val yearMonth = YearMonth.parse(date, formatter)

        return when (action) {
            Action.INCREASE -> yearMonth.plusMonths(value)
